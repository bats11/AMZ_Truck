import {
  ...
  bindClipPlane
} from "./chunk-IIYSGVJX.js";
import {
  AutoReleaseWorkerPool,
  RGBDTextureTools,
  _UpdateRGBDAsync
} from "./chunk-AC5CZF2W.js";
import {
  AbstractScene,
  ArrayTools,
  Axis,
...
var Bone = class _Bone extends Node {
   ...
   */
  constructor(name41, skeleton, parentBone = null, localMatrix = null, restMatrix = null, bindMatrix = null, index = null) {
    super(name41, skeleton.getScene());
    this.name = name41;
    this.children = [];
    this.animations = [];
    this._index = null;
...
Mesh._instancedMeshFactory = (name41, mesh) => {
  const instance = new InstancedMesh(name41, mesh);
...
var InstancedMesh = class extends AbstractMesh {
   ...
   */
  constructor(name41, source) {
    super(name41, source.getScene());
   ...
   */
  createInstance(name41) {
    return this._sourceMesh.createInstance(name41);
  }
  /**
   ...
   * @returns the clone
   */
  clone(name41, newParent = null, doNotCloneChildren, newSourceMesh) {
    const result = (newSourceMesh || this._sourceMesh).createInstance(name41);
...
var AssetContainer = class extends AbstractScene {
  ...
  mergeAnimationsTo(scene = EngineStore.LastCreatedScene, animatables, targetConverter = null) {
    ...
    const _targetConverter = targetConverter ? targetConverter : (target) => {
      ...
      const name41 = target.name.split(".").join("").split("_primitive")[0];
      switch (targetProperty) {
        case "position":
        case "rotationQuaternion":
          node = scene.getTransformNodeByName(target.name) || scene.getTransformNodeByName(name41);
          break;
        case "influence":
          node = scene.getMorphTargetByName(target.name) || scene.getMorphTargetByName(name41);
          break;
        default:
          node = scene.getNodeByName(target.name) || scene.getNodeByName(name41);
      }
      return node;
    };
...
var Sound = class _Sound {
   ...
   */
  constructor(name41, urlOrArrayBuffer, scene, readyToPlayCallback = null, options) {
    var _a;
    this.autoplay = false;
    this._loop = false;
...
var Skeleton = class _Skeleton {
  ...
  toString(fullDetails) {
    ...
    if (fullDetails) {
      ret += ", Ranges: {";
      let first = true;
      for (const name41 in this._ranges) {
        if (first) {
          ret += ", ";
          first = false;
        }
   ...
   */
  getBoneIndexByName(name41) {
    for (let boneIndex = 0, cache = this.bones.length; boneIndex < cache; boneIndex++) {
      if (this.bones[boneIndex].name === name41) {
        return boneIndex;
      }
    }
    return -1;
   ...
   */
  createAnimationRange(name41, from, to) {
    if (!this._ranges[name41]) {
      this._ranges[name41] = new AnimationRange(name41, from, to);
      for (let i = 0, nBones = this.bones.length; i < nBones; i++) {
        if (this.bones[i].animations[0]) {
          this.bones[i].animations[0].createRange(name41, from, to);
        }
      }
    }
  }
   ...
   */
  deleteAnimationRange(name41, deleteFrames = true) {
    for (let i = 0, nBones = this.bones.length; i < nBones; i++) {
      if (this.bones[i].animations[0]) {
        this.bones[i].animations[0].deleteRange(name41, deleteFrames);
      }
    }
    this._ranges[name41] = null;
  }
  /**
   ...
   */
  getAnimationRange(name41) {
    return this._ranges[name41] || null;
  }
  /**
  ...
  getAnimationRanges() {
    ...
    let name41;
    for (name41 in this._ranges) {
      animationRanges.push(this._ranges[name41]);
    }
   ...
   */
  copyAnimationRange(source, name41, rescaleAsRequired = false) {
    if (this._ranges[name41] || !source.getAnimationRange(name41)) {
      return false;
    }
    let ret = true;
    ...
    for (i = 0, nBones = this.bones.length; i < nBones; i++) {
      ...
      if (sourceBone) {
        ret = ret && this.bones[i].copyAnimationRange(sourceBone, name41, frameOffset, rescaleAsRequired, skelDimensionsRatio);
      } else {
        Logger.Warn("copyAnimationRange: not same rig, missing source bone " + boneName);
        ret = false;
      }
    }
    const range = source.getAnimationRange(name41);
    if (range) {
      this._ranges[name41] = new AnimationRange(name41, range.from + frameOffset, range.to + frameOffset);
    }
    return ret;
  }
  /**
   ...
   */
  beginAnimation(name41, loop, speedRatio, onAnimationEnd) {
    const range = this.getAnimationRange(name41);
    if (!range) {
      return null;
    }
   ...
   */
  clone(name41, id) {
    const result = new _Skeleton(name41, id || name41, this._scene);
  ...
  serialize() {
    ...
    for (let index = 0; index < this.bones.length; index++) {
      ...
      for (const name41 in this._ranges) {
        const source = this._ranges[name41];
        if (!source) {
          continue;
        }
        const range = {};
...
Node.AddNodeConstructor("TargetCamera", (name41, scene) => {
  return () => new TargetCamera(name41, Vector3.Zero(), scene);
});
var TargetCamera = class _TargetCamera extends Camera {
   ...
   */
  constructor(name41, position, scene, setActiveOnSceneIfNoneActive = true) {
    super(name41, position, scene, setActiveOnSceneIfNoneActive);
  ...
  createRigCamera(name41, cameraIndex) {
    if (this.cameraRigMode !== Camera.RIG_MODE_NONE) {
      const rigCamera = new _TargetCamera(name41, this.position.clone(), this.getScene());
...
Node.AddNodeConstructor("Light_Type_1", (name41, scene) => {
  return () => new DirectionalLight(name41, Vector3.Zero(), scene);
});
var DirectionalLight = class extends ShadowLight {
   ...
   */
  constructor(name41, direction, scene) {
    super(name41, scene);
...
Node.AddNodeConstructor("Light_Type_2", (name41, scene) => {
  return () => new SpotLight(name41, Vector3.Zero(), Vector3.Zero(), 0, 0, scene);
});
var SpotLight = class _SpotLight extends ShadowLight {
   ...
   */
  constructor(name41, position, direction, angle, exponent, scene) {
    super(name41, scene);
    this._innerAngle = 0;
...
var name2 = "pbrUboDeclaration";
var shader2 = `layout(std140,column_major) uniform;uniform Material {vec2 vAlbedoInfos;vec4 vAmbientInfos;vec2 vOpacityInfos;vec2 vEmissiveInfos;vec2 vLightmapInfos;vec3 vReflectivityInfos;vec2 vMicroSurfaceSamplerInfos;vec2 vReflectionInfos;vec2 vReflectionFilteringInfo;vec3 vReflectionPosition;vec3 vReflectionSize;vec3 vBumpInfos;mat4 albedoMatrix;mat4 ambientMatrix;mat4 opacityMatrix;mat4 emissiveMatrix;mat4 lightmapMatrix;mat4 reflectivityMatrix;mat4 microSurfaceSamplerMatrix;mat4 bumpMatrix;vec2 vTangentSpaceParams;mat4 reflectionMatrix;vec3 vReflectionColor;vec4 vAlbedoColor;vec4 vLightingIntensity;vec3 vReflectionMicrosurfaceInfos;float pointSize;vec4 vReflectivityColor;vec3 vEmissiveColor;vec3 vAmbientColor;vec2 vDebugMode;vec4 vMetallicReflectanceFactors;vec2 vMetallicReflectanceInfos;mat4 metallicReflectanceMatrix;vec2 vReflectanceInfos;mat4 reflectanceMatrix;vec3 vSphericalL00;vec3 vSphericalL1_1;vec3 vSphericalL10;vec3 vSphericalL11;vec3 vSphericalL2_2;vec3 vSphericalL2_1;vec3 vSphericalL20;vec3 vSphericalL21;vec3 vSphericalL22;vec3 vSphericalX;vec3 vSphericalY;vec3 vSphericalZ;vec3 vSphericalXX_ZZ;vec3 vSphericalYY_ZZ;vec3 vSphericalZZ;vec3 vSphericalXY;vec3 vSphericalYZ;vec3 vSphericalZX;
...
var name3 = "pbrFragmentExtraDeclaration";
var shader3 = `varying vec3 vPositionW;
#if DEBUGMODE>0
...
var name4 = "samplerFragmentAlternateDeclaration";
...
var name5 = "pbrFragmentSamplersDeclaration";
var shader5 = `#include<samplerFragmentDeclaration>(_DEFINENAME_,ALBEDO,_VARYINGNAME_,Albedo,_SAMPLERNAME_,albedo)
...
var name6 = "subSurfaceScatteringFunctions";
var shader6 = `bool testLightingForSSS(float diffusionProfile)
...
var name8 = "pbrHelperFunctions";
var shader8 = `#define MINIMUMVARIANCE 0.0005
...
var name9 = "harmonicsFunctions";
var shader9 = `#ifdef USESPHERICALFROMREFLECTIONMAP
#ifdef SPHERICAL_HARMONICS
...
var name10 = "pbrDirectLightingSetupFunctions";
var shader10 = `struct preLightingInfo
...
var name11 = "pbrDirectLightingFalloffFunctions";
var shader11 = `float computeDistanceLightFalloff_Standard(vec3 lightOffset,float range)
...
var name12 = "pbrBRDFFunctions";
var shader12 = `#define FRESNEL_MAXIMUM_ON_ROUGH 0.25
...
var name13 = "hdrFilteringFunctions";
var shader13 = `#ifdef NUM_SAMPLES
#if NUM_SAMPLES>0
...
var name14 = "pbrDirectLightingFunctions";
var shader14 = `#define CLEARCOATREFLECTANCE90 1.0
struct lightingInfo
{vec3 diffuse;
#ifdef SPECULARTERM
vec3 specular;
...
var name15 = "pbrIBLFunctions";
var shader15 = `#if defined(REFLECTION) || defined(SS_REFRACTION)
...
var name16 = "pbrBlockAlbedoOpacity";
var shader16 = `struct albedoOpacityOutParams
...
var name17 = "pbrBlockReflectivity";
var shader17 = `struct reflectivityOutParams
...
var name18 = "pbrBlockAmbientOcclusion";
var shader18 = `struct ambientOcclusionOutParams
{vec3 ambientOcclusionColor;
...
var name19 = "pbrBlockAlphaFresnel";
var shader19 = `#ifdef ALPHAFRESNEL
...
var name20 = "pbrBlockAnisotropic";
var shader20 = `#ifdef ANISOTROPIC
...
var name21 = "pbrBlockReflection";
var shader21 = `#ifdef REFLECTION
...
var name22 = "pbrBlockSheen";
var shader22 = `#ifdef SHEEN
...
var name23 = "pbrBlockClearcoat";
var shader23 = `struct clearcoatOutParams
...
var name24 = "pbrBlockIridescence";
var shader24 = `struct iridescenceOutParams
...
var name25 = "pbrBlockSubSurface";
var shader25 = `struct subSurfaceOutParams
...
var name26 = "pbrBlockNormalGeometric";
var shader26 = `vec3 viewDirectionW=normalize(vEyePosition.xyz-vPositionW);
#ifdef NORMAL
...
var name27 = "pbrBlockNormalFinal";
var shader27 = `#if defined(FORCENORMALFORWARD) && defined(NORMAL)
...
var name28 = "pbrBlockLightmapInit";
var shader28 = `#ifdef LIGHTMAP
...
var name29 = "pbrBlockGeometryInfo";
var shader29 = `float NdotVUnclamped=dot(normalW,viewDirectionW);float NdotV=absEps(NdotVUnclamped);float alphaG=convertRoughnessToAverageSlope(roughness);vec2 AARoughnessFactors=getAARoughnessFactors(normalW.xyz);
#ifdef SPECULARAA
...
var name30 = "pbrBlockReflectance0";
var shader30 = `float reflectance=max(max(reflectivityOut.surfaceReflectivityColor.r,reflectivityOut.surfaceReflectivityColor.g),reflectivityOut.surfaceReflectivityColor.b);vec3 specularEnvironmentR0=reflectivityOut.surfaceReflectivityColor.rgb;
#ifdef METALLICWORKFLOW
...
var name31 = "pbrBlockReflectance";
var shader31 = `#if defined(ENVIRONMENTBRDF) && !defined(REFLECTIONMAP_SKYBOX)
...
var name32 = "pbrBlockDirectLighting";
var shader32 = `vec3 diffuseBase=vec3(0.,0.,0.);
#ifdef SPECULARTERM
...
var name33 = "pbrBlockFinalLitComponents";
var shader33 = `aggShadow=aggShadow/numLights;
#if defined(ENVIRONMENTBRDF)
#ifdef MS_BRDF_ENERGY_CONSERVATION
...
var name34 = "pbrBlockFinalUnlitComponents";
var shader34 = `vec3 finalDiffuse=diffuseBase;finalDiffuse*=surfaceAlbedo.rgb;finalDiffuse=max(finalDiffuse,0.0);finalDiffuse*=vLightingIntensity.x;vec3 finalAmbient=vAmbientColor;finalAmbient*=surfaceAlbedo.rgb;vec3 finalEmissive=vEmissiveColor;
#ifdef EMISSIVE
...
var name35 = "pbrBlockFinalColorComposition";
var shader35 = `vec4 finalColor=vec4(
#ifndef UNLIT
#ifdef REFLECTION
finalIrradiance +
#endif
...
var name36 = "pbrBlockImageProcessing";
var shader36 = `#if defined(IMAGEPROCESSINGPOSTPROCESS) || defined(SS_SCATTERING)
...
var name38 = "pbrPixelShader";
var shader38 = `#if defined(BUMP) || !defined(NORMAL) || defined(FORCENORMALFORWARD) || defined(SPECULARAA) || defined(CLEARCOAT_BUMP) || defined(ANISOTROPIC)
...
var name39 = "pbrVertexDeclaration";
var shader39 = `uniform mat4 view;uniform mat4 viewProjection;
#ifdef ALBEDO
...
var name40 = "pbrVertexShader";
var shader40 = `precision highp float;
...
var PBRMaterial = class _PBRMaterial extends PBRBaseMaterial {
   ...
   */
  clone(name41, cloneTexturesOnlyOnce = true, rootUrl = "") {
    const clone = SerializationHelper.Clone(() => new _PBRMaterial(name41, this.getScene()), this, { cloneTexturesOnlyOnce });
    clone.id = name41;
    clone.name = name41;
...
Node.AddNodeConstructor("Light_Type_0", (name41, scene) => {
  return () => new PointLight(name41, Vector3.Zero(), scene);
});
var PointLight = class extends ShadowLight {
   ...
   * @param scene The scene the lights belongs to
   */
  constructor(name41, position, scene) {
    super(name41, scene);
...
var MorphTarget = class _MorphTarget {
   ...
   */
  constructor(name41, influence = 0, scene = null) {
    this.name = name41;
    this.animations = [];
   ...
   * @returns a new MorphTarget
   */
  static FromMesh(mesh, name41, influence) {
    if (!name41) {
      name41 = mesh.name;
    }
    const result = new _MorphTarget(name41, influence, mesh.getScene());
...
var DracoCompression = class _DracoCompression {
   ...
   */
  async decodeMeshToGeometryAsync(name41, scene, data, attributes) {
    const meshData = await this._decodeMeshAsync(data, attributes);
    const geometry = new Geometry(name41, scene);
    if (meshData.indices) {
  ...
  /** @internal */
  async _decodeMeshToGeometryForGltfAsync(name41, scene, data, attributes, gltfNormalizedOverride) {
...
var FlowGraphConnection = class {
  constructor(name41, _connectionType, _ownerBlock) {
...
var FlowGraphDataConnection = class extends FlowGraphConnection {
   ...
   * @param richType
   */
  constructor(name41, connectionType, ownerBlock, richType) {
    super(name41, connectionType, ownerBlock);
...
var FlowGraphBlock = class {
   ...
   */
  registerDataInput(name41, richType) {
    const input = new FlowGraphDataConnection(name41, FlowGraphConnectionType.Input, this, richType);
   ...
   * @returns the created connection
   */
  registerDataOutput(name41, richType) {
    const output = new FlowGraphDataConnection(name41, FlowGraphConnectionType.Output, this, richType);
   ...
   */
  getDataInput(name41) {
    return this.dataInputs.find((i) => i.name === name41);
  }
  /**
   ...
   * @returns the connection if it exists, undefined otherwise
   */
  getDataOutput(name41) {
    return this.dataOutputs.find((i) => i.name === name41);
  }
  /**
...
var FlowGraphExecutionBlock = class extends FlowGraphBlock {
  ...
  _registerSignalInput(name41) {
    const input = new FlowGraphSignalConnection(name41, FlowGraphConnectionType.Input, this);
    this.signalInputs.push(input);
    return input;
  }
  _registerSignalOutput(name41) {
    const output = new FlowGraphSignalConnection(name41, FlowGraphConnectionType.Output, this);
   ...
   */
  getSignalInput(name41) {
    return this.signalInputs.find((input) => input.name === name41);
  }
  /**
   ...
   * @returns if the input exists, the input. Otherwise, undefined.
   */
  getSignalOutput(name41) {
    return this.signalOutputs.find((output) => output.name === name41);
  }
  /**
   * Serializes this block
...
var FlowGraphContext = class {
   ...
   */
  hasVariable(name41) {
    return name41 in this._userVariables;
  }
  /**
   ...
   */
  setVariable(name41, value) {
    this._userVariables[name41] = value;
  }
  /**
   ...
   */
  getVariable(name41) {
    return this._userVariables[name41];
  }
  /**
  ...
  _getUniqueIdPrefixedName(obj, name41) {
    return `${obj.uniqueId}_${name41}`;
  }
  /**
   ...
   * @param value
   */
  _setExecutionVariable(block, name41, value) {
    this._executionVariables[this._getUniqueIdPrefixedName(block, name41)] = value;
  }
  /**
   ...
   * @returns
   */
  _getExecutionVariable(block, name41, defaultValue) {
    if (this._hasExecutionVariable(block, name41)) {
      return this._executionVariables[this._getUniqueIdPrefixedName(block, name41)];
    } else {
      return defaultValue;
    }
  }
   ...
   * @param name
   */
  _deleteExecutionVariable(block, name41) {
    delete this._executionVariables[this._getUniqueIdPrefixedName(block, name41)];
  }
  /**
   ...
   * @returns
   */
  _hasExecutionVariable(block, name41) {
    return this._getUniqueIdPrefixedName(block, name41) in this._executionVariables;
  }
  /**
...
//# sourceMappingURL=chunk-UBLHLSBL.js.map
